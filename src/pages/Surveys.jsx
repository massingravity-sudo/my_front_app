...
export default function Surveys() {
    ...
    const loadSurveys = async () => {
        try {
            const r = await surveysAPI.getAll();
            // FIX: s'assurer que r.data est bien un tableau
            setSurveys(Array.isArray(r.data) ? r.data : []);
        } catch (e) {
            console.error(e);
            setSurveys([]);
        } finally {
    ...
    const handleOpenSurvey = async (survey) => {
        ...
        try {
            ...
            // FIX: s'assurer que questions existe avant d'utiliser .length
            setAnswers(new Array(r.data.questions?.length ?? 0).fill(''));
        } catch (e) {
            console.error(e);
        }
    };
    ...
    // FIX: protection contre deadline null/undefined
    const getStatusColor = (survey) => {
        ...
        if (!survey.deadline) return 'bg-gray-100 text-gray-800 border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600';
    ...
    // FIX: protection contre deadline null pour l'affichage
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {/* FIX: user?.role au lieu de user.role pour éviter crash si user = null */}
                        {user?.role === 'admin' && (
                            <button
                    ...
                    {surveys.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {surveys.map(survey => {
                                // FIX: protection questions undefined
                                    ...
                                    >
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex-1">
                                                <h3 className={`text-xl font-bold mb-2 ${d.text}`}>{survey.title ?? 'Sans titre'}</h3>
                                        ...
                                        <div className={`flex items-center text-sm ${d.textMuted} gap-4`}>
                                            {/* FIX: formatDeadline protégé */}
                                            <div className="flex items-center gap-1">
                                                <Clock className="w-4 h-4" />
                                                <span>Jusqu'au {formatDeadline(survey.deadline)}</span>
                                            </div>
                                            {/* FIX: responseCount protégé */}
                                            <div className="flex items-center gap-1">
                                        ...
                                        {!survey.has_responded && (
                                            <button
                                                onClick={e => { e.stopPropagation(); handleOpenSurvey(survey); }}
            ...
            {selectedSurvey && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
                    <div className={`${d.modal} rounded-2xl p-8 w-full max-w-3xl my-8 border ${d.divider}`}>
                        <div className="mb-6">
                            <h2 className={`text-2xl font-bold mb-2 ${d.text}`}>{selectedSurvey.title ?? 'Sans titre'}</h2>
                        ...
                        <div className="space-y-6 mb-6 max-h-[60vh] overflow-y-auto">
                            {/* FIX: protection si questions est undefined */}
                                ...
                                <div key={i} className={`border rounded-lg p-4 ${d.divider} ${darkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                                    ...
                                    {q.type === 'scale' && (
                                        <div className="flex items-center gap-2 flex-wrap">
                                            {/* FIX: protection q.max undefined */}
                                            {[...Array(q.max ?? 5)].map((_, k) => (
                                                <button
                                                    key={k}
                                                    onClick={() => handleAnswerChange(i, String(k + 1))}
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className={`flex-1 px-6 py-3 border rounded-lg font-semibold transition ${d.cancelBtn}`}
                            >
                                Annuler
                            </button>
                            <button
